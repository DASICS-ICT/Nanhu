...
class ExceptionGen(wbNum:Int)(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper {
  ...
  private val selector = Module(new SelectPolicy(wbNum, true, false))
  selector.io.in.zip(writebackGroup).foreach({case(a, b) =>
    a.valid := b.valid
    a.bits := b.bits.robIdx
  })
  private val oldestSel = selector.io.out
  private val writebackSel = Mux1H(oldestSel.bits, writebackGroup.map(_.bits))
  ...
  private val s1_out_valid = oldestSel.valid
  private val s1_out_bits = writebackSel
